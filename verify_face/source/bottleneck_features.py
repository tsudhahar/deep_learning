...
class Features():
    def __init__(self, dataset_path, img_width=224, img_height=224, face_crop = True,
        ...
        self.dataset = get_lfw_dataset(dataset_path,features_dir,min_images_per_label,max_images_per_label)
        self.random_draw_test = get_random_lfw(dataset_path,features_dir,200, min_images_per_label/2)
        if mode == 'softmax':
    ...
    def get_codes(self,dataset,feature_file_name, label_file_name):
        ...
        for id in tqdm(walk(dataset), total = len(dataset),unit='id'):
            ...
            label_batch = np.full((len(features_batch), 1), id.id)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--face_align', type=str, action='store', default='True', dest='face_align',
                        help='whether to align and crop face')
    ...
    feature = Features(args.input_dir, 224, 224, face_crop=args.face_align, min_images_per_label=args.min_images_per_label,
