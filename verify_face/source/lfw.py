...
def get_lfw_dataset(input_directory,output_directory,min_images_per_label=10,max_images_per_label = 1000):
    dataset = []
    ...
    ##debug
    file_name = os.path.join(output_directory, 'lfw_dataset.csv')
...
def get_random_lfw(input_directory,output_directory,num_pictures,min_images_per_label=20):
    dataset = []
    ids = os.listdir(input_directory)
    ids.sort()
    num_classes = len(ids)
    ...
    file_name = os.path.join(output_directory, 'lfw_random_draw.csv')
